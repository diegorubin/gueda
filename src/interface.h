...
void gerar_win_principal() {
	...
	dir_pixmaps = g_strdup_printf("%s/.gravit/pixmaps/",g_get_home_dir());
	...
	g_signal_connect (G_OBJECT (winPrincipal), "delete_event", G_CALLBACK (gtk_main_quit) , NULL);
	...
	gtk_table_attach (GTK_TABLE(tblPrincipal), tblBack1, 1, 2, 1, 5, GTK_FILL, GTK_SHRINK, 0, 0);
	...
	gtk_table_attach (GTK_TABLE(tblPrincipal), tblBack2, 5, 6, 1, 5, GTK_FILL, GTK_SHRINK, 0, 0);
